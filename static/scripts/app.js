...
document.addEventListener('DOMContentLoaded', () => {
  // Function to update total price and button text on product detail page
  function updateTotalPrice() {
    const quantityInput = document.querySelector('#quantity');
    const totalPriceElement = document.querySelector('#total-price');
    const addToCartText = document.querySelector('.add-to-cart-text');

    // Only update if we're on a product detail page with the right elements
    if (quantityInput && totalPriceElement && totalPriceElement.dataset.unitPrice) {
      const quantity = parseInt(quantityInput.value) || 1;
      const unitPrice = parseInt(totalPriceElement.dataset.unitPrice) || 0;

      if (unitPrice > 0) {
        const newTotal = unitPrice * quantity;
        totalPriceElement.textContent = newTotal.toLocaleString() + ' تومان';

        // Update add to cart button text to show quantity
        if (addToCartText) {
          const cartonInfo = document.querySelector('#carton-info');
          if (cartonInfo && !cartonInfo.classList.contains('hidden')) {
            // Carton product
            if (quantity === 1) {
        ...
        // Update carton info if it's a carton product
        const cartonInfo = document.querySelector('#carton-info');
        if (cartonInfo) {
          const cartonCountSpan = cartonInfo.querySelector('span');
          if (cartonCountSpan) {
            cartonCountSpan.textContent = quantity;
          }
        }

        // Update carton detail if exists
        const cartonDetail = document.querySelector('#carton-detail');
        if (cartonDetail) {
          try {
            const memberCount = parseInt(cartonDetail.textContent.match(/(\d+) عدد/)[1]) || 1;
            cartonDetail.textContent = `${quantity} کارتن × ${memberCount} عدد × ${unitPrice.toLocaleString()}`;
          } catch (e) {
            // Ignore regex errors
          }
        }
  ...
  // Event Listener for Increment Buttons (skip product detail pages - handled by dedicated script)
  document.querySelectorAll('.increment').forEach(button => {
    button.addEventListener('click', event => {
      // Skip if we're on a product detail page (handled by product/script.html)
      if (document.querySelector('#total-price')) {
        return;
      }

      // Handle SVG elements - event.target might be the <use> element
      let targetElement = event.target;
      if (targetElement.tagName === 'use') {
        targetElement = targetElement.closest('svg');
      }

      // Look for the input in different possible locations
      let input = null;

      // Try to find input in the same button container (for product-details.html)
      const buttonContainer = targetElement.closest('button');
      if (buttonContainer) {
        input = buttonContainer.querySelector('.custom-input');
      }

      // If not found, try to find input in flex container (for other pages)
      if (!input) {
        const flexContainer = targetElement.closest('.flex');
        if (flexContainer) {
          input = flexContainer.querySelector('.custom-input');
        }
      }

      // If still not found, try to find by ID (for specific cases)
      if (!input) {
        input = document.querySelector('#quantity') || document.querySelector('#customInput');
      }
  ...
  // Event Listener for Decrement Buttons (skip product detail pages - handled by dedicated script)
  ...
  // Also listen for direct input changes on quantity field
  const quantityInput = document.querySelector('#quantity');
  if (quantityInput) {
    quantityInput.addEventListener('input', updateTotalPrice);
  }

  // Initialize total price display (only on product detail pages)
  if (document.querySelector('#total-price')) {
    updateTotalPrice();
  }
});
...
document.querySelectorAll(".price-slider").forEach((sliderContainer) => {
  ...
  if (priceElements.length >= 2 && rangeInputs.length >= 2) {
  ...
  rangeInputs.forEach((input) => {
    input.addEventListener("input", (e) => {
      ...
      if (maxVal - minVal < priceGap) {
        if (e.target.classList.contains("price-range-min")) {
